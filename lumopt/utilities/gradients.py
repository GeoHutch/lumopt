...
class GradientFields(object):
    ...
    def __init__(self,
                 forward_fields: "Fields",
                 adjoint_fields: "Fields",
                 ):
    ...
    def plot(self, fig, ax_forward, ax_gradients, ax_adjoint, original_grid=True):
        ax_forward.clear()
        self.forward_fields.plot(ax_forward, title='Forward Fields', cmap='Blues')
        self.plot_gradients(fig, ax_gradients, original_grid)
        self.adjoint_fields.plot(ax_adjoint, title='Adjoint Fields', cmap='Blues')
    ...
    def boundary_perturbation_integrand(self):
        ...
        def gradient_field(x, y, z, wl, normal, eps_in, eps_out):
            ...
            result = (2.0 * sp.constants.epsilon_0 * (eps_in - eps_out) * np.sum(
                E_parallel_forward * E_parallel_adjoint)
                      + (1.0 / eps_out - 1.0 / eps_in) / sp.constants.epsilon_0 * np.sum(
                        D_perp_forward * D_perp_adjoint))
    ...
    def spatial_gradient_integral_on_cad(sim,
        ...
        # print(f'partial_fom_derivs_vs_lambda')
