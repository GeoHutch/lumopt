...
class Fields(object):
    ...
    def scale(self, dimension, factors):
        ...
        if hasattr(self.E, 'dtype'):
            if self.E.shape[dimension] == len(factors):
                self.E = np.concatenate([np.take(self.E, [index], axis=dimension) * factors[index] for index in
                                         range(self.E.shape[dimension])], axis=dimension)
                self.getfield = self.make_field_interpolation_object(self.E)
            else:
        ...
        if hasattr(self.D, 'dtype'):
            if self.D.shape[dimension] == len(factors):
                self.D = np.concatenate([np.take(self.D, [index], axis=dimension) * factors[index] for index in
                                         range(self.D.shape[dimension])], axis=dimension)
                self.getDfield = self.make_field_interpolation_object(self.D)
            else:
        ...
        if hasattr(self.H, 'dtype'):
            if self.H.shape[dimension] == len(factors):
                self.H = np.concatenate([np.take(self.H, [index], axis=dimension) * factors[index] for index in
                                         range(self.H.shape[dimension])], axis=dimension)
                self.getHfield = self.make_field_interpolation_object(self.H)
            else:
    ...
    def make_field_interpolation_object(self, F):
        ...
        Fx_interpolator = wrapped_GridInterpolator((self.x, self.y, self.z, wl), F[:, :, :, :, 0], method='linear',
                                                   bounds_error=False)
        Fy_interpolator = wrapped_GridInterpolator((self.x, self.y, self.z, wl), F[:, :, :, :, 1], method='linear',
                                                   bounds_error=False)
        Fz_interpolator = wrapped_GridInterpolator((self.x, self.y, self.z, wl), F[:, :, :, :, 2], method='linear',
                                                   bounds_error=False)
...
class FieldsNoInterp(Fields):
    ...
    def __init__(self, x, y, z, wl, deltas, E, D, eps, H):
        ...
        x, y, z, wl, delta_x, delta_y, delta_z = map(self.process_array_shape,
                                                     [x, y, z, wl, delta_x, delta_y, delta_z])
    ...
    def make_field_interpolation_object_nointerp(self, F):
        if (F.shape[3] == 1) and (len(self.wl) > 1):
            Fx_interpolator = wrapped_GridInterpolator(points=(self.x + self.deltas[0], self.y, self.z, self.wl[0]),
                                                       values=np.take(F, indices=[0], axis=4),
                                                       method='linear',
                                                       bounds_error=False)
            Fy_interpolator = wrapped_GridInterpolator(points=(self.x, self.y + self.deltas[1], self.z, self.wl[0]),
                                                       values=np.take(F, indices=[1], axis=4),
                                                       method='linear',
                                                       bounds_error=False)
            Fz_interpolator = wrapped_GridInterpolator(points=(self.x, self.y, self.z + self.deltas[2], self.wl[0]),
                                                       values=np.take(F, indices=[2], axis=4),
                                                       method='linear',
                                                       bounds_error=False)
        else:
            Fx_interpolator = wrapped_GridInterpolator((self.x + self.deltas[0], self.y, self.z, self.wl),
                                                       np.take(F, indices=[0], axis=4),
                                                       method='linear',
                                                       bounds_error=False)
            Fy_interpolator = wrapped_GridInterpolator((self.x, self.y + self.deltas[1], self.z, self.wl),
                                                       np.take(F, indices=[1], axis=4),
                                                       method='linear',
                                                       bounds_error=False)
            Fz_interpolator = wrapped_GridInterpolator((self.x, self.y, self.z + self.deltas[2], self.wl),
                                                       np.take(F, indices=[2], axis=4),
                                                       method='linear',
                                                       bounds_error=False)
