        ...
        if hasattr(optimization, 'optimizations'):
            for i, opt in enumerate(optimization.optimizations):
                if hasattr(opt, 'gradient_fields'):
                    ...
                    opt.gradient_fields.plot(self.fig, self.ax[1, 1], self.ax[0, 1], self.ax[1, 0])
                print('Plots updated with optimization {0} iteration {1} results'.format(i,
                                                                                         optimization.optimizer.iteration - 1))
        else:
            if hasattr(optimization, 'gradient_fields'):
                ...
                optimization.gradient_fields.plot(self.fig, self.ax[1, 1], self.ax[0, 1], self.ax[1, 0])
