...
class Polygon(Geometry):
    """ 
        Defines a polygon with vertices on the (x,y)-plane that are extruded along the z direction to create a 3-D
        shape. The vertices are defined as a numpy array of coordinate pairs np.array([(x0,y0),...,(xn,yn)]).
        THE VERTICES MUST BE ORDERED IN A COUNTER CLOCKWISE DIRECTION.
        ...
        :param edge_precision: number of quadrature points along each edge for computing the FOM gradient using the
                                shape derivative approximation method.
    """
    ...
    def make_edges(self):
        ...
        for i, point in enumerate(self.points):
            edges.append(Edge(self.points[i - 1], self.points[i], eps_in=self.eps_in, eps_out=self.eps_out, z=self.z,
                              depth=self.depth))
        self.edges = edges
    ...
    def calculate_gradients(self, gradient_fields):
        ...
        for edge in self.edges:
            ...
            sys.stdout.write('.')
        print('')
        # the gradients returned for an edge derivative are the gradients with
        # respect to moving each end point perpendicular to that edge
        ...
        for i, point in enumerate(self.points):
            ...
            deriv_x = np.dot([1, 0, 0],
                             (np.outer(normal_edge_1, deriv_edge_1).squeeze() +
                              np.outer(normal_edge_2, deriv_edge_2).squeeze()))
            deriv_y = np.dot([0, 1, 0],
                             (np.outer(normal_edge_1, deriv_edge_1).squeeze() +
                              np.outer(normal_edge_2, deriv_edge_2).squeeze()))
    ...
    def update_geometry(self, points_linear, sim=None):
        """
        Sets the points. Must be fed a linear array of points,
        because during the optimization the point coordinates are not by pair
        """
...
class FunctionDefinedPolygon(Polygon):
    """ 
        Constructs a polygon from a user defined function that takes the optimization parameters and returns a set of
        vertices defining a polygon.
        The polygon vertices returned by the function must be defined as a numpy array of coordinate pairs
        np.array([(x0,y0),...,(xn,yn)]).
        THE VERTICES MUST BE ORDERED IN A COUNTER CLOCKWISE DIRECTION.

        Parameters
        ----------
        :param func:           function that takes the optimization parameter values and returns a polygon.
